...
def apply_mask(x, mask1, mask2, num_p, stage, branch):
    ...
    if num_p == 38:
        ...
        w = Multiply(name=w_name)([x, mask2])  # heat_weight
    return w

def get_training_model(weight_decay, is_binary = True):
    ...
    for sn in range(2, stages + 1):
        ...
        stageT_branch1_out = stageT_block(x, np_branch1, sn, 1, weight_decay, is_binary)
        w1 = apply_mask(stageT_branch1_out, vec_weight_input, heat_weight_input, np_branch1, sn, 1)

        # stage SN - branch 2 (confidence maps)
        stageT_branch2_out = stageT_block(x, np_branch2, sn, 2, weight_decay, is_binary)
...
def get_testing_model(is_binary = True):
    if(is_binary):
        print("binary weight")
    stages = 6
    np_branch1 = 38
    ...
    for sn in range(2, stages + 1):
        stageT_branch1_out = stageT_block(x, np_branch1, sn, 1, None, is_binary)
        stageT_branch2_out = stageT_block(x, np_branch2, sn, 2, None, is_binary)
